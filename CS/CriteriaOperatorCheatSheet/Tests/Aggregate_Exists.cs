...
namespace dxTestSolutionXPO.Tests {
    ...
    public class Aggregate_Exists : BaseTest {
        ...
        public void Task0_0() {
            ...
            var resultCriterion = new BinaryOperator(nameof(Order.OrderName), "FirstName0");
            var result3 = uow.Evaluate<Order>(criterion, resultCriterion);
            //assert
        ...
        public void Task0_0_false() {
            ...
            var criterion = 
                CriteriaOperator.Parse("OrderItems.Exists()");
            var resultCriterion = new BinaryOperator(nameof(Order.OrderName), "FirstName1");
            var result3 = uow.Evaluate<Order>(criterion, resultCriterion);
            //assert
        ...
        public void Task1_0_false() {
            ...
            //act
            CriteriaOperator criterion = 
                CriteriaOperator.Parse("[OrderItems][ItemPrice=30]");
            CriteriaOperator filterParentCollection = new BinaryOperator(nameof(Order.OrderName), "FirstName2");
            var result3 = uow.Evaluate<Order>(criterion, filterParentCollection);
            //assert
            Assert.AreEqual(false, result3);
